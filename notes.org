...
* Ninth day
  ...
  construct four different kinds of servers (parallel, sequential,
...
*** Analysis
    ...
    call. There's a small solace in that a smart enough GC being able
    to reclaim this memory in a relative small frame of time, because
    you're only ever pointing to the most recent binary string. Note
    how if the function were not tail recursive, you would have to keep
    /all the binary strings/ in memory, a comparable sin to the fact
    that they were written to memory in the first place.
    ...
    Go run this, and feel how fast it is. The entirety of google is
    brought to you in that little text (that's a lie, actually,
    because this page references a bunch of scripts from other pages,
    but that's not relevant for my point). Isn't that great? I'm not
...
** Writing a tcp server
   Let's just muse for a bit about how we might want to write a
   server. What's the abstraction that we want to present to the user
   of the library?
   
   When interacting with a TCP server, we're only starting a
   connection, sending bytes, receiving bytes, and closing the
   connection. A TCP library should then make these very simple.

   #+begin_src erlang
     % All these functions will register the server as `tcp_server` for
     % message passing purposes.

     % Starts a server listening on Port, and then, upon receiving a
     % connection, creates a thread Fun with the accepted socket with
     % default arguments [binary, {packet, 0}] as default server options
     -spec start_server(Host, Port, Fun).

     % Starts a server listening on Port, and then, upon receiving a
     % connection, creates a thread Fun with the accepted socket as an
     % argument with PortOptions options.
     -spec start_server(Host, Port, Fun, PortOptions).

     % Starts a server with a limited thread pool size. Will only start
     % that number of threads, and the rest of the connections will not be
     % acknowledged while all the threads are busy. 0 for unlimited pool
     % size.
     -spec start_server(Host, Port, Fun, PortOptions, PoolSize).

     % Kills the server and unbinds the socket
     -spec stop_server().
   #+end_src
